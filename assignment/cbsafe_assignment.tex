...
\author{\IEEEauthorblockN{Wahiduzzaman Suva, Esme Moula Chowdhury Abha}
...
We review 20 peer-reviewed works from top venues (IEEE S\&P, ACM CCS, USENIX
Security, NDSS, NeurIPS, ICML, and IEEE Transactions), grouped into four themes and
compared critically in Table~\ref{tab:lit}: for each we state \emph{what} the work
does and \emph{how}, and its \emph{key limitation} relative to our goal.
...
\begin{tikzpicture}[
  >={Latex[length=2.2mm]},
  block/.style={draw, rounded corners, align=center, minimum height=1.25cm,
                text width=2.7cm, inner sep=3pt, font=\footnotesize},
  setup/.style={block, fill=orange!12},
  core/.style={block, fill=blue!8},
  det/.style={block, fill=green!12},
  node distance=0.7cm]
\node[setup] (kem) {\textbf{1. One-time}\\\textbf{HQC key setup}\\code-based KEM};
\node[core, right=of kem] (train) {\textbf{2. Local training}\\\textbf{+ masking}\\per client};
\node[core, right=of train] (clus) {\textbf{3. Cluster secure}\\\textbf{aggregation} ($c$)\\server sees $k$ cluster sums};
\node[core, right=of clus] (rob) {\textbf{4. Robust}\\\textbf{aggregation}\\across cluster sums};
\node[det, right=of rob] (det) {\textbf{5. CB-SAFE+}\\temporal group testing\\exclude attackers};
\draw[->] (kem)--(train);
\draw[->] (train)--(clus);
\draw[->] (clus)--(rob);
\draw[->] (rob)--(det);
\draw[->] (det.south) -- ++(0,-0.6)
  -| node[pos=0.25, below, font=\scriptsize] {next round: re-randomized clustering} (train.south);
\end{tikzpicture}
\caption{CB-SAFE pipeline (one round). After a one-time code-based (HQC) key setup,
clients train locally and mask their updates; secure aggregation \emph{within} clusters
of size $c$ exposes only $k$ cluster sums to the server; a robust rule aggregates
\emph{across} those sums to update the global model; and CB-SAFE+ accumulates
per-client suspicion by temporal group testing to exclude attackers before the next,
re-randomized round.}
\label{fig:arch}
\end{figure*}
